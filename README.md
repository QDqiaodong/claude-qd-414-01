...
1. **库间 cell**（软删）：编号唯一；温区（冷冻/冷藏）；容量。软删后 `list` 查不到，但其货位仍可关联。
   - **容量变更是可审计动作**：新容量不能小于「已在库箱数 + 仍待入箱数 + 已确认未核销预占箱数」，低于下限、负数、空容量（含旧页面提交 / 绕过页面直连 / 坏请求体）一律后端明确驳回，原容量、库存、预占状态不变；驳回信息带齐三项占用与原容量。
   - **同一串行点**：容量变更与预占确认、待入批次开立都先对库间行加写锁（`READ_COMMITTED`，锁内实时汇总）。容量变更先提交，后到的预占确认按新剩余容量判定并被准确驳回；预占确认先提交，后到的容量变更把该预占计入下限而被驳回，绝不留「提示成功却覆盖新预占」或「预占已确认而容量低于承诺量」。
   - 保存成功后容量总览、预占页、批次页都经 `capacity/overview` 这一唯一入口立即换成新口径；页面输入框 min 仅为体验提示，不是防线。
