...
1. **库间 cell**（软删）：编号唯一；温区（冷冻/冷藏）；容量。软删后 `list` 查不到，但其货位仍可关联。
   - **容量变更是可审计的硬约束**（不只靠输入框 min/按钮禁用）：`PUT /api/cells/{id}` 在同一事务里先对库间行加悲观写锁，再按最新数据校验
     `新容量 ≥ 已在库箱数 + 仍待入箱数 + 已确认未核销预占箱数`；
     空容量、负数、低于下限一律 400 明确驳回（驳回信息逐项列出撑住下限的批次/预占单号），原容量、库存、预占状态一字不动；
     容量变更与预占确认共用「库间行锁 + READ_COMMITTED」这一个串行点——容量先改小，后到的确认按新剩余被驳回；预占先确认，后到的容量变更把它计入下限被驳回。
     开立/修改待入批次同样先锁库间行，与容量变更、预占确认同序排队。
